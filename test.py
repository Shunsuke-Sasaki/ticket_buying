...
class TestTest():
  ...
  def PssportJudgeAndCilck(day, when, notbuy_xpath, click_xpath):
    if len(self.driver.find_elements_by_xpath(notbuy_xpath)) > 0:
      NotBuyElement = self.driver.find_element_by_xpath(notbuy_xpath)
  ...
  def wait():
    ...
    Webself.driverWait(self.driver, 2000).until(EC.invisibility_of_element_located((By.ID, 'loading_modal0overlay')))
    time.sleep(5)
...
#軽く待機させる関数
  ...
  def test_test(self):
    self.self.driver.get("https://reserve.tokyodisneyresort.jp/")
    #ログインクリック
    self.driver.find_element_by_xpath('//*[@id="header"]/div/ul[2]/li/a/img').click()
    wait()  
    LoginId = self.driver.find_element_by_xpath('//*[@id="_userId"]')
    LoginId.send_keys("<自分のディズニーログインアカウント名>")
    password = self.driver.find_element_by_xpath('//*[@id="_password"]')
    password.send_keys("<自分のディズニーログインパスワード>")
    self.driver.find_element_by_xpath('//*[@id="_loginConection"]/form/p/a/img').click()
    wait()
    #トラベルバッグクリック
    self.driver.find_element_by_xpath('//*[@id="header"]/div/ul[3]/li[2]/a/img').click()
    wait()

    #チケットを追加するクリック
    self.driver.find_element_by_xpath('//*[@id="dayTable"]/tbody/tr[1]/td/table/tbody/tr/td[2]/a').click()
    wait()

    # ＞クリック
    self.driver.find_element_by_xpath('//*[@id="searchCalendar"]/div/div/ul/button[2]').click()
    wait()
    ...
    while True:
      ...
      ActionHover = ActionChains(self.driver)
      ActionHover.move_to_element(self.driver.find_element_by_xpath(\
      ...
      #「自宅でプリントアウト」のクリック
      self.driver.find_element_by_xpath('//*[@id="searchEticket"]').click()
      wait()
